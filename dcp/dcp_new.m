...
% .........................................................................
  case 'init'
    if (nargin == 3) 
      ...
      if (nargin == 5)
          dcps(ID).mode = varargin{4};
      else
          dcps(ID).mode = 'normal';
      end
      dcps(ID).c_order = 0;
      if (nargin == 6)
        dcps(ID).c_order = varargin{5};
      end
      ...
      % the avg and deviation for decoupled weighted dmp
      dcps(ID).u = 0.5;
      dcps(ID).sigma = 0.05; 
...
%     alphax = dcps(ID).alpha_x;
    ...
    if (dcps(ID).c_order == 1),
      ...
      dcps(ID).vd = 0;
      dcps(ID).xd = (alphax*(0-dcps(ID).x)+cc)*tau*cc_tau;
    end
    
%     scaling = dcps(ID).g - dcps(ID).y0;
scaling = 1;
    switch dcps(ID).mode
        case 'bio_inspired'
            K = dcps(ID).alpha_z * dcps(ID).beta_z;
            D = dcps(ID).alpha_z;
            dcps(ID).zd = in * K * (f / (in + 1e-10) + dcps(ID).y0 - dcps(ID).y) + (1 - in) * K * (dcps(ID).g - dcps(ID).y) - D * dcps(ID).z;
        case 'normal'
            dcps(ID).zd = (dcps(ID).alpha_z*(dcps(ID).beta_z*(dcps(ID).g-dcps(ID).y)-dcps(ID).z)+f*scaling+ct)*tau*ct_tau;
        case 'decoupled_weight'
            fw = f/ (in + 1e-10);
            
            wg = 0.5 * (1 + erf( (curt - dcps(ID).u )/ (dcps(ID).sigma * sqrt(2))));
            
            K = dcps(ID).alpha_z * dcps(ID).beta_z;
            D = dcps(ID).alpha_z;
            dcps(ID).zd = (1 - wg) * (fw + dcps(ID).y0 - dcps(ID).y) + wg * K * (dcps(ID).g - dcps(ID).y) - D * dcps(ID).z;            
        case 'ccdmp_normal'
            g_t = 1;
            a0 = dcps(ID).G0 - g_t;
            dcps(ID).zd = (dcps(ID).alpha_z*(dcps(ID).beta_z*(g_t + a0 -dcps(ID).y)-dcps(ID).z)+f+ct)*tau*ct_tau;
%             dcps(ID).zd = (dcps(ID).alpha_z*(dcps(ID).beta_z*(dcps(ID).g-dcps(ID).y)-dcps(ID).z)+f+ct)*tau*ct_tau;

        case 'ccdmp_bio_inspired'
            g_t = 1;
            a0 = dcps(ID).G0 - g_t;
            K = dcps(ID).alpha_z * dcps(ID).beta_z;
            D = dcps(ID).alpha_z;
            dcps(ID).zd = in * K * (f / (in + 1e-10) + dcps(ID).y0 - dcps(ID).a0 - dcps(ID).y + a0) + (1 - in) * K * (dcps(ID).G0 - dcps(ID).y) - D * dcps(ID).z;
    end
...
%      
    if strcmp(dcps(ID).mode, 'ccdmp_normal') || strcmp(dcps(ID).mode, 'ccdmp_bio_inspired')
        a1 = dcps(ID).G - g_t;
        a0 = dcps(ID).G0 - g_t;
%         a_1 = dcps(ID).G_1 - g_t;
        dcps(ID).y = dcps(ID).y +  (a1 - a0);
%         dcps(ID).yd = dcps(ID).yd + (a1 - a0) / dt;
%         dcps(ID).ydd = dcps(ID).ydd + ((a1 - a0) / dt - (a0 - a_1)/dt)/dt;
    end
...
%     amp = dcps(ID).g - dcps(ID).y0;
amp = 1;
    switch dcps(ID).mode
        case 'bio_inspired'
            K = dcps(ID).alpha_z * dcps(ID).beta_z;
            D = dcps(ID).alpha_z;
            ft = (tdd / tau^2 - K * (dcps(ID).g - t) + D * td / tau + K * (dcps(ID).g - dcps(ID).y0) * in) / K;
            ft = ft / amp;
        case 'normal'
        ...
        case 'decoupled_weight'
            ...
            ft = 1 / (1 - wg + 1e-10) * (tdd / tau^2 - wg *  K * (dcps(ID).g - t) + D * td / tau ) - dcps(ID).y0 + t;
            ft = ft * in / amp;
    end
    % the weighted sum of the locally weighted regression models
    dcps(ID).psi = exp(-0.5*((dcps(ID).x-dcps(ID).c).^2).*dcps(ID).D);
...
%     alphax = dcps(ID).alpha_x;
    ...
    if (dcps(ID).c_order == 1),
      ...
      dcps(ID).vd = 0;
      dcps(ID).xd = alphax*(0-dcps(ID).x)*tau;
    end
    ...
    switch dcps(ID).mode
        case 'bio_inspired'
            K = dcps(ID).alpha_z * dcps(ID).beta_z;
            D = dcps(ID).alpha_z;
            dcps(ID).zd = in * K * (f / (in + 0.00001) + dcps(ID).y0 - dcps(ID).y) + (1 - in) * K * (dcps(ID).g - dcps(ID).y) - D * dcps(ID).z;
        case 'normal'
...
%       Tdd              = diffnc(Td,dt);
    ...
    switch dcps(ID).mode
        case 'bio_inspired'
            K = dcps(ID).alpha_z * dcps(ID).beta_z;
            D = dcps(ID).alpha_z;
            Ft = (Tdd / tau^2 - K * (dcps(ID).g - T) + D * Td / tau + K * (dcps(ID).g - dcps(ID).y0) * X) / K;
            Ft = Ft / amp;
        case 'normal'
            Ft = (Tdd/tau^2-dcps(ID).alpha_z*(dcps(ID).beta_z*(dcps(ID).G-T)-Td/tau))/amp;
